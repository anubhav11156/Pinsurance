...
contract Pool {
    ...
    function stake(
        ...
        require(
            !userPoolAccountStatus[userAddress].haveStaked,
            "Already Staked!"
        );
        ...
        if (poolMembers.length == 2) {
            // means all user have staked their amount
    ...
    function createClaimRequest(
        ...
        if (isUserMember(_userAddress)) {
            ...
            pinsuranceContract.createClaim(
                _userAddress,
                address(this),
                docUri,
                POOL_NAME,
    ...
    function approveClaim(address claimerAddress) public {
        ...
        require(
            !currentClaim.poolMembersApprovalStatus[msg.sender],
            "Already approved!"
        );
    ...
    function declineClaim(address claimerAddress) public {
        userClaimDetails[claimerAddress].poolMembersApprovalStatus[
            msg.sender
        ] = false;
    }
    ...
    function claimFund(uint256 amount) public {
        require(
            (block.timestamp > poolData.from) &&
                (block.timestamp < poolData.to),
            "Claim request not in pool period!"
        );
        require(
            userClaimDetails[msg.sender].isApproved,
        ...
        require(
            mockUsdcContract.balanceOf(address(this)) > amount,
    ...
    function executePool() external returns (bool) {
        require(owner == msg.sender, "Unauthorized");
        require(block.timestamp > poolData.to, "Pool still active.");

        FakeUSDC mockUsdcContract = FakeUSDC(FAKE_USDC_ADDRESS);
        require(
            mockUsdcContract.balanceOf(address(this)) > 0,
            "Insufficient pool balance"
        );

        uint totalPoolMembers = poolMembers.length;
        uint equalDivideFunds = mockUsdcContract.balanceOf(address(this)) /
            totalPoolMembers;

        for (uint i = 0; i < totalPoolMembers; i++) {
            address member = poolMembers[i];
            if (userClaimDetails[member].claimed == false) {
                payable(member).transfer(equalDivideFunds);
            }
        }
    ...
    function getMemberApprovalStatus(
